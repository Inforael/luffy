...
Descricao da utilizacao do for:
    ...
    baseado na quantidade de posiC'C5es da segunda linha.
...
lista de variaves:
    ...
    -length_array: quantidade de posiC'C5es na segunda linha do arquivos. Pula-se a linha de cabeC'alho;
...
for source_file in source_file_list:
    try:
        ...
        #Remove acentuacao dos arquivos
        read_csv_open = read_csv_open.apply(lambda x: x.str.normalize('NFD').str.encode('ascii', errors='ignore').str.decode('utf-8'))
        #sobrescreve arquivos com dados sem acentuacao
        read_csv_open.to_csv(source_file, index=None, sep='\n')
        ...
        erro=str("Arquivo Rejeitado nB: {}: {} tamanho: {:,.0f} erro: {}".format(empty_file_count,short_name_file,bytes_size/float(1<<10),type(exception).__name__))
        ...
        if length_array in length_array_file_dict:
            ...
            length_array_file_dict[length_array]=[source_file]
            erro="Arquivo Rejeitado nB: {}: {} tamanho: {:,.0f} erro: {}".format(empty_file_count,short_name_file,length_array/float(1<<10),type(exception).__name__)            
            LOG_EMPTY_FILES.write(erro+'\n')
            erro_ctrl_file=str("||||||||||||||{}||||||||".format(short_name_file))
            CTRL_FILE_LOG.write(erro_ctrl_file+'\n')
        empty_file_count+=1
...
lista de argumentos:
    ...
    -to_path:
        caminho full onde o csv serC! gravado.
...
def fia_csv_909(file_list,to_path):
    ...
    for flat_file in file_list:
        ...
        #posiC'C5es das colunas para o arquivo de 910 posiC'C5es
